...
const ModalButton = styled.button<{ $primary?: boolean }>`
  ...
  font-size: 0.85rem;
  background: ${(p) =>
    p.$primary ? "var(--ifm-color-primary, #4a9)" : "transparent"};
...
export const SpectrogramSettingsModal: React.FC<
  SpectrogramSettingsModalProps
> = ({ open, onClose, config, colorMap, onApply }) => {
    ...
    <StyledDialog ref={dialogRef}>
      ...
      <FormGrid>
        ...
        <Field>
          ...
          >
            {[
              ...
              {
                label: `${Math.floor(fftSize / 2)} (50%)`,
                value: Math.floor(fftSize / 2),
              },
              {
                label: `${Math.floor(fftSize / 4)} (75%)`,
                value: Math.floor(fftSize / 4),
              },
              {
                label: `${Math.floor(fftSize / 8)} (87.5%)`,
                value: Math.floor(fftSize / 8),
              },
            ].map((o) => (
        ...
        <Field>
          ...
          <Select
            value={windowFn}
            onChange={(e) => setWindowFn(e.target.value as typeof windowFn)}
          >
