    ...
    async (
      ...
      try {
        ...
        if ((effectsFunction || hasOfflineTrackEffects) && applyEffects) {
          ...
          const totalClips = tracksToRender.reduce(
...
async function renderWithToneEffects(
  ...
  const { Offline, Volume, Gain, Panner, Player, ToneAudioBuffer } =
    await import("tone");
...
async function scheduleClip(
  ...
  if (!audioBuffer) {
    console.warn(
      `Skipping clip "${clip.name || clip.id}" - no audioBuffer for export`,
    );
    return;
  }
...
function applyFadeEnvelope(
  ...
  switch (fadeType) {
    ...
    case "logarithmic":
      ...
      const logCurve = generateFadeCurve(
        startValue,
        endValue,
        256,
        "logarithmic",
