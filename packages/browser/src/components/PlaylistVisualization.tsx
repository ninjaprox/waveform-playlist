...
export const PlaylistVisualization: React.FC<PlaylistVisualizationProps> = ({
  ...
  const perTrackSpectrogramHelpers = useMemo(() => {
    ...
    tracks.forEach((track) => {
      ...
      helpers.set(track.id, {
        ...
        frequencyScaleFn: spectrogram.getFrequencyScale(
          cfg?.frequencyScale ?? "mel",
        ),
        config: cfg,
      });
    });
  ...
  const [settingsModalTrackId, setSettingsModalTrackId] = useState<
    string | null
  >(null);
  ...
  let displayDuration =
    audioBuffers.length > 0 ? duration : DEFAULT_EMPTY_TRACK_DURATION;

  if (recordingState?.isRecording) {
    const recordingEndSample =
      recordingState.startSample + recordingState.durationSamples;
    const recordingEndTime = recordingEndSample / sampleRate;
    displayDuration = Math.max(displayDuration, recordingEndTime + 10);
  }

  const tracksFullWidth = Math.floor(
    (displayDuration * sampleRate) / samplesPerPixel,
  );
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    for (let i = 0; i < peaksDataArray.length; i++) {
      ...
      if (
        trackY >= cumulativeHeight &&
        trackY < cumulativeHeight + trackHeight
      ) {
        clickedTrackIndex = i;
        break;
      }
            ...
            timeScaleHeight > 0 ? (
              <>
                ...
                {isLoopEnabled && (
                  <TimescaleLoopRegion
                    startPosition={
                      (Math.min(loopStart, loopEnd) * sampleRate) /
                      samplesPerPixel
                    }
                    endPosition={
                      (Math.max(loopStart, loopEnd) * sampleRate) /
                      samplesPerPixel
                    }
                    ...
                    onLoopRegionChange={(startPixels, endPixels) => {
                      const startSeconds =
                        (startPixels * samplesPerPixel) / sampleRate;
                      const endSeconds =
                        (endPixels * samplesPerPixel) / sampleRate;
          ...
          <>
            {peaksDataArray.map((trackClipPeaks, trackIndex) => {
              ...
              const effectiveRenderMode =
                spectrogram?.trackSpectrogramOverrides.get(track.id)
                  ?.renderMode ??
                track.renderMode ??
                "waveform";
                ...
                <Controls onClick={() => selectTrack(trackIndex)}>
                        ...
                        <TrackMenu
                          items={(onClose) =>
                            spectrogram.renderMenuItems!({
                              ...
                              onOpenSettings: () =>
                                setSettingsModalTrackId(track.id),
                              onClose,
                            })
                          }
                        />
                  ...
                  <ButtonGroup>
                    <Button
                      ...
                      onClick={() =>
                        setTrackMute(trackIndex, !trackState.muted)
                      }
                    >
                      Mute
                    </Button>
                    <Button
                      ...
                      onClick={() =>
                        setTrackSolo(trackIndex, !trackState.soloed)
                      }
                    >
                      Solo
                    </Button>
                ...
                >
                  ...
                  >
                    {effectiveRenderMode !== "waveform" &&
                      (() => {
                        const helpers = perTrackSpectrogramHelpers.get(
                          track.id,
                        );
                        ...
                        >
                          {peaksData.data.map(
                            (channelPeaks: Peaks, channelIndex: number) => {
                              const clipSpectrograms =
                                spectrogram?.spectrogramDataMap.get(
                                  clip.clipId,
                                );
                              const channelSpectrogram =
                                clipSpectrograms?.[channelIndex] ??
                                clipSpectrograms?.[0];
                              const helpers = perTrackSpectrogramHelpers.get(
                                track.id,
                              );
                              ...
                              return (
                                <ChannelWithProgress
                                  ...
                                  spectrogramFrequencyScaleFn={
                                    helpers?.frequencyScaleFn
                                  }
                                  spectrogramMinFrequency={
                                    trackCfg?.minFrequency
                                  }
                                  spectrogramMaxFrequency={
                                    trackCfg?.maxFrequency
                                  }
            ...
            {(isPlaying || selectionStart === selectionEnd) &&
              (renderPlayhead ? (
                renderPlayhead({
                  position:
                    ((currentTimeRef.current ?? 0) * sampleRate) /
                      samplesPerPixel +
                    (controls.show ? controls.width : 0),
          ...
          <spectrogram.SettingsModal
            ...
            config={
              settingsModalTrackId !== null
                ? (spectrogram.trackSpectrogramOverrides.get(
                    settingsModalTrackId,
                  )?.config ??
                  tracks.find((t) => t.id === settingsModalTrackId)
                    ?.spectrogramConfig ??
                  spectrogram.spectrogramConfig ??
                  {})
                : {}
            ...
            onApply={(newConfig, newColorMap) => {
              if (settingsModalTrackId !== null) {
                spectrogram.setTrackSpectrogramConfig(
                  settingsModalTrackId,
                  newConfig,
                  newColorMap,
                );
