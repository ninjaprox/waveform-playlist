...
export const SpectrogramProvider: React.FC<SpectrogramProviderProps> = ({
  ...
  const { tracks, waveHeight, samplesPerPixel, isReady, mono, controls } =
    usePlaylistData();
  const { scrollContainerRef } = usePlaylistControls();

  // State
  const [spectrogramDataMap, setSpectrogramDataMap] = useState<
    Map<string, SpectrogramData[]>
  >(new Map());
    ...
    if (!workerApi) {
      try {
        const rawWorker = new Worker(
          new URL(
            "@waveform-playlist/spectrogram/worker/spectrogram.worker",
            import.meta.url,
          ),
          { type: "module" },
        );
  ...
  useEffect(() => {
    ...
    tracks.forEach((track) => {
      const mode =
        trackSpectrogramOverrides.get(track.id)?.renderMode ??
        track.renderMode ??
        "waveform";
      ...
      currentFFTKeys.set(
        track.id,
        JSON.stringify({ mode, mono, ...computeConfig }),
      );
    });
    ...
    const canvasVersionChanged =
      spectrogramCanvasVersion !== prevCanvasVersionRef.current;
    ...
    if (!workerApi) {
      try {
        ...
        console.warn(
          "Spectrogram Web Worker unavailable, falling back to synchronous computation",
        );
      }
    }
    ...
    tracks.forEach((track, i) => {
      ...
      const trackFFTChanged =
        fftKeyChanged &&
        currentFFTKeys.get(track.id) !== prevFFTKeys.get(track.id);
      const hasRegisteredCanvases =
        canvasVersionChanged &&
        track.clips.some((clip) =>
          spectrogramCanvasRegistryRef.current.has(clip.id),
        );
      ...
      for (const clip of track.clips) {
        ...
        if (
          !trackFFTChanged &&
          !hasRegisteredCanvases &&
          clipCacheKeysRef.current.has(clip.id)
        ) {
    ...
    if (clipsNeedingFFT.length === 0 && clipsNeedingDisplayOnly.length === 0)
      return;
    ...
    if (!workerApi) {
      try {
        ...
        console.error(
          "[waveform-playlist] Synchronous spectrogram computation failed:",
          err,
        );
      }
      return;
    ...
    const getVisibleChunkRange = (
      ...
      if (!container) {
        return {
          visibleIndices: canvasWidths.map((_, i) => i),
          remainingIndices: [],
        };
      }
    ...
    const computeAsync = async () => {
      ...
      for (const item of clipsNeedingFFT) {
        if (
          spectrogramGenerationRef.current !== generation ||
          abortToken.aborted
        )
          return;

        try {
          const clipCanvasInfo = spectrogramCanvasRegistryRef.current.get(
            item.clipId,
          );
          if (clipCanvasInfo && clipCanvasInfo.size > 0) {
            const numChannels = item.monoFlag
              ? 1
              : item.channelDataArrays.length;
            const clipPixelOffset = Math.floor(
              item.clipStartSample / samplesPerPixel,
            );
            ...
            if (container) {
              ...
              const clipEndPx =
                clipStartPx + Math.ceil(item.durationSamples / samplesPerPixel);
              ...
              if (overlapEndPx > overlapStartPx) {
                ...
                const visStartSample =
                  item.offsetSamples +
                  Math.floor(localStartPx * samplesPerPixel);
            ...
            const fullClipAlreadyCached = clipCacheKeysRef.current.has(
              item.clipId,
            );
            ...
            for (let ch = 0; ch < numChannels; ch++) {
              ...
              for (
                let batchStart = 0;
                batchStart < remainingIndices.length;
                ...
                const batch = remainingIndices.slice(
                  batchStart,
                  batchStart + BATCH_SIZE,
                );
      ...
      for (const item of clipsNeedingDisplayOnly) {
        ...
        try {
          ...
          console.warn(
            "Spectrogram display re-render error for clip",
            item.clipId,
            err,
          );
        }
  ...
  // Setters
  const setTrackRenderMode = useCallback(
    (trackId: string, mode: RenderMode) => {
  ...
  const unregisterSpectrogramCanvases = useCallback(
    (clipId: string, channelIndex: number) => {
      const registry = spectrogramCanvasRegistryRef.current;
  ...
  const value: SpectrogramIntegration = useMemo(
    () => ({
      spectrogramDataMap,
      trackSpectrogramOverrides,
      spectrogramWorkerApi: spectrogramWorkerReady
        ? spectrogramWorkerRef.current
        : null,
      spectrogramConfig,
      spectrogramColorMap,
      setTrackRenderMode,
